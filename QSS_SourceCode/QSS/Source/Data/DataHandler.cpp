...
DataHandler::DataHandler() : Thread ("Load Audio Thread")
{
    ...
    overviewPlayback.reset(new OverviewPlayback(&oschandler));
...
void DataHandler::loadData(bool loadInThread)
{
    ...
    {
        File agentDataFile(DEFAULT_DATAFILE);
        if(BATCH_SONIFY_TEST)
        {
            agentDataFile = RESOURCE_PATH + "/pg1260_norm.ascii";
        }
...
void DataHandler::retrieveRawData(File dataFile)
{
    ...
    while (! inputStream.isExhausted()) // [3]
    {
        ...
        {
            StringArray sArray;
            sArray.addTokens(line, delimiter, "");
            RawData rd;
            
            rd.wavelength = sArray[0].getFloatValue();
            rd.flux = sArray[1].getFloatValue();
            
            // NOTE: We are capping flux values at 5, as large values can cause our routine to hang. This doesn't affect us because we are mainly concerned with flux values less than 1, as that indicates absorption.
            if(rd.flux > 5)
            {
                rd.flux = 5;
            }
    ...
    std::cout << "    -- num entries: " << rawWavelengthData.size() << std::endl;
...
void DataHandler::loadAtomicDataList()
{
    ...
    // load the entire atomic list into a vector
    while (! inputStream.isExhausted()) // [3]
    {
        String line = inputStream.readNextLine();
    ...
    assignFamilyIDToAtom();
    
    
    if(BATCH_SONIFY_TEST)
    {
        // atomic list used for sonifying system in Tripp paper.
        filterAtomicList_pg1260_Tripp();
    }
    else
    {
        // atomic list used for JAES paper...
        filterAtomicList_TON580();
    }
...
void DataHandler::assignFamilyIDToAtom()
{
    for(int i = 0; i < atomicTransitions.size(); i++)
    {
        String atomName = atomicTransitions[i].name;
        
        if(atomName == "H I")
        {
            atomicTransitions[i].familyID = eFamilyID_1;
        }
        else if(atomName == "MgI"
                || atomName == "FeI"
                || atomName == "SiI"
                || atomName == "AlI"
                || atomName == "S I"
                || atomName == "C I")
        {
            atomicTransitions[i].familyID = eFamilyID_2;
        }
        else if(atomName == "AlII"
                || atomName == "N I"
                || atomName == "SiII"
                || atomName == "S II"
                || atomName == "O I")
        {
            atomicTransitions[i].familyID = eFamilyID_3;
        }
        else if(atomName == "SiIII"
                || atomName == "S III"
                || atomName == "N II"
                || atomName == "C II"
                || atomName == "O II")
        {
            atomicTransitions[i].familyID = eFamilyID_4;
        }
        else if(atomName == "C  III"
                || atomName == "S IV"
                || atomName == "O III"
                || atomName == "N III")
        {
            atomicTransitions[i].familyID = eFamilyID_5;
        }
        else if(atomName == "O  IV"
                || atomName == "O V"
                || atomName == "NeVII")
        {
            atomicTransitions[i].familyID = eFamilyID_6;
        }
    }
...
//    {
//
//        if(atomicTransitions[i].familyID != -999) continue;
//        if(atomNames.contains(atomicTransitions[i].name)) continue;
//
//        atomNames.add(atomicTransitions[i].name);
//        std::cout << "atom name: " << atomicTransitions[i].name << ". and familyID: " << atomicTransitions[i].familyID << std::endl;
//
//    }
...
void DataHandler::filterAtomicList_TON580()
{
    // filter on the data entries, only allow those within the following thresholds...
    for(int i = 0; i < atomicTransitions.size(); i++)
    {
        if(!(atomicTransitions[i].absorptionProbability >= 9.75
             && atomicTransitions[i].restframeRate >= 900
             && atomicTransitions[i].restframeRate <= 1800))
        {
            atomicTransitions.erase(atomicTransitions.begin() + i);
            i--;
        }
    }
    

    // generate the atomic spectral list (a 2d vector - each row = an atom, each column is the atom's rest frame rate info)
    ...
    for(int i = 0; i < atomicTransitions.size(); i++)
    {
        ...
        for(int j = i; j < atomicTransitions.size(); j++)
        {
            if(checkName == atomicTransitions[j].name)
            {
                ...
                sd.familyID = atomicTransitions[j].familyID;
...
void DataHandler::filterAtomicList_pg1260_Tripp()
{
        // filter on the data entries, only allow those within the following thresholds...
        for(int i = 0; i < atomicTransitions.size(); i++)
        {
            if(!(atomicTransitions[i].name == "MgII"
                 || atomicTransitions[i].name == "N III"
                 || atomicTransitions[i].name == "N IV"
                 || atomicTransitions[i].name == "N V"
                 || atomicTransitions[i].name == "NeVIII"
                 || atomicTransitions[i].name == "MgX"
                 || atomicTransitions[i].name == "O III"
                 || atomicTransitions[i].name == "O IV"
                 || atomicTransitions[i].name == "S III"
                 || atomicTransitions[i].name == "S IV"
                 || atomicTransitions[i].name == "S V"
                 || atomicTransitions[i].name == "H I"))
            {
                atomicTransitions.erase(atomicTransitions.begin() + i);
                i--;
            }
        ...
        for(int i = 0; i < atomSpectraList.size(); i++)
        {
            ...
            {
                // MgII
                if(atomSpectraList[i][0].label == "MgII")
                {
                    if(!(atomSpectraList[i][j].restFrameRate == 2796.3520))
                    {
                        atomSpectraList[i].erase(atomSpectraList[i].begin() + j);
                        j--;
                    }
                ...
                // N III
                if(atomSpectraList[i][0].label == "N III")
                {
                    if(!(atomSpectraList[i][j].restFrameRate == 684.9960))
                    {
                        atomSpectraList[i].erase(atomSpectraList[i].begin() + j);
                        j--;
                    }
                ...
                // N IV
                if(atomSpectraList[i][0].label == "N IV")
                {
                    if(!(atomSpectraList[i][j].restFrameRate == 765.1480))
                    {
                        atomSpectraList[i].erase(atomSpectraList[i].begin() + j);
                        j--;
                    }
                ...
                // MgX
                if(atomSpectraList[i][0].label == "MgX")
                {
                    if(!(atomSpectraList[i][j].restFrameRate == 624.950))
                    {
                        atomSpectraList[i].erase(atomSpectraList[i].begin() + j);
                        j--;
                    }
                ...
                // O III
                if(atomSpectraList[i][0].label == "O III")
                {
                    if(!(atomSpectraList[i][j].restFrameRate == 702.332))
                    {
                        atomSpectraList[i].erase(atomSpectraList[i].begin() + j);
                        j--;
                    }
                ...
                // O IV
                if(atomSpectraList[i][0].label == "O IV")
                {
                    if(!(atomSpectraList[i][j].restFrameRate == 787.711))
                    {
                        atomSpectraList[i].erase(atomSpectraList[i].begin() + j);
                        j--;
                    }
                ...
                // S III
                if(atomSpectraList[i][0].label == "S III")
                {
                    if(!(atomSpectraList[i][j].restFrameRate == 698.7310))
                    {
                        atomSpectraList[i].erase(atomSpectraList[i].begin() + j);
                        j--;
                    }
                ...
                // S IV
                if(atomSpectraList[i][0].label == "S IV")
                {
                    if(!(atomSpectraList[i][j].restFrameRate == 657.340))
                    {
                        atomSpectraList[i].erase(atomSpectraList[i].begin() + j);
                        j--;
                    }
                ...
                // S V
                if(atomSpectraList[i][0].label == "S V")
                {
                    if(!(atomSpectraList[i][j].restFrameRate == 786.480))
                    {
                        atomSpectraList[i].erase(atomSpectraList[i].begin() + j);
                        j--;
                    }
...
void DataHandler::setBatchSpectralDataForSonification()
{
    
    selectedBatchSpectralData.clear();
    for(int i = 0; i < atomSpectraList.size(); i++)
    {
        std::vector<SpectralData> sd = atomSpectraList[i];
        selectedBatchSpectralData.push_back(sd);
    }
    
    
    // process the batch of atomic data spectral transitions for sonification
    for(int i = 0; i < selectedBatchSpectralData.size(); i++)
    {
        // compute the flux based on the input velocity with anchored red shift...

        float velocity = 0;
        float redShift = 0.927;
        getFluxFromVelocity(selectedBatchSpectralData[i], velocity, redShift);


        // compute the sonification parameters
        setSonificationSpectralAmplitudes(selectedBatchSpectralData[i]);
        setSonificationBandwidthParameters(selectedBatchSpectralData[i]);
        setACDFrequency(selectedBatchSpectralData[i]);

        broadcastDataOSC(selectedBatchSpectralData[i]);
    }
    
    
    // process and broadcast a random spectrum from our selected batch.
//    Random r;
//    int rIndx = r.nextInt((const int)selectedBatchSpectralData.size());
//
//    float velocity = 0;
//    float redShift = 0.927;
//    getFluxFromVelocity(selectedBatchSpectralData[rIndx], velocity, redShift);
//
//
//    // compute the sonification parameters
//    setSonificationSpectralAmplitudes(selectedBatchSpectralData[rIndx]);
//    setSonificationBandwidthParameters(selectedBatchSpectralData[rIndx]);
//    setACDFrequency(selectedBatchSpectralData[rIndx]);
//
//    // broadcast the spectral data via osc
//    broadcastDataOSC(selectedBatchSpectralData[rIndx]);
    
    
    
    // set the playback parameters for broadcasting via osc to Max/MSP to play the sound...
    overviewPlayback->familyPlayback.clear();
    ...
    for(int i = 0; i < selectedBatchSpectralData.size(); i++)
    {
        ...
        OverviewPlayback::PlaybackParameters playPrm;
        ...
        for(int j = 0; j < specData.size(); j++)
        {
            ...
            if(playPrm.familyID == eFamilyID_1)
            {
                
                if(r.nextFloat() < 0.333)
                {
                    pitch = 36;
                }
                else if(r.nextFloat() < 0.666)
                {
                    pitch = 48;
                }
                else
            ...
            else if(playPrm.familyID == eFamilyID_4)
            {
                pitch = 60 + r.nextInt(7);
                
                attack = 50 * r.nextFloat() + 10;
                release = 200 * r.nextFloat() + 50;
                
                pan = 0;
            }
            else if(playPrm.familyID == eFamilyID_5)
            {
                 pitch = 72 + r.nextInt(7);
            ...
            // release
            // stereo pan
            String metricsString = String(pitch)
                                   + " " + String(1 - specData[j].flux)
                                   + " " + String(attack)
                                   + " " + String(0)
                                   + " " + String(1)
                                   + " " + String(release)
        ...
        overviewPlayback->familyPlayback.push_back(playPrm);
    }
    
    
    for(int i = 0; i < overviewPlayback->familyPlayback.size(); i++)
    {
        std::cout << "------ playback family: " << overviewPlayback->familyPlayback[i].familyID << std::endl;
        std::cout << "------ playback address: " << overviewPlayback->familyPlayback[i].familyAddress << std::endl;
        
        for(int j = 0; j < overviewPlayback->familyPlayback[i].metricStrings.size(); j++)
        {
            std::cout << "     ---- metric string: " << overviewPlayback->familyPlayback[i].metricStrings[j] << std::endl;
        }
    }
...
float DataHandler::boundaryAtHalfMaximum(int startIndx, SpectralData &specData)
{
    ...
    float redShift = specData.redShift; //0.20257;
